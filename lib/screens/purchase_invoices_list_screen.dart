...
class _PurchaseInvoicesListScreenState extends State<PurchaseInvoicesListScreen> {
  Future<void> _deleteInvoice(BuildContext context, PurchaseInvoice invoice) async {
    ...
    try {
      ...
      if (partyDoc.exists) {
        ...
        if (partyData != null) {
          ...
          double newBalance = currentBalance + invoice.total;
      ...
      await FirebaseService.deletePurchaseInvoice(invoice.id);
  ...
  Future<void> _downloadPdf(BuildContext context, PurchaseInvoice invoice) async {
    try {
      showDialog(
        context: context,
        barrierDismissible: false,
