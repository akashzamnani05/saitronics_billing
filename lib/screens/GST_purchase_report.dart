...
import 'package:saitronics_billing/utils/GST_purchase_report_generator.dart';

class GSTPurchaseReport extends StatefulWidget {
  const GSTPurchaseReport({Key? key}) : super(key: key);

  @override
  State<GSTPurchaseReport> createState() => _GSTPurchaseReportState();
}

class _GSTPurchaseReportState extends State<GSTPurchaseReport> {
  ...
  try {
    ...
    String? filePath;
      if (format == 'excel') {
        filePath = await PurchaseReportExcelExporter.exportToExcel(
        ...
        filePath = await PurchaseReportPdfGenerator.generatePdf(
          invoices: _filteredInvoices,
          partyCache: _partyCache,
          startDate: startDate,
          endDate: endDate,
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              _buildFilterSection(),
              Expanded(
                child: StreamBuilder<List<PurchaseInvoice>>(
      ...
      child: SingleChildScrollView(
        child: DataTable(
          ...
          columns: const [
            ...
            DataColumn(label: Text('Original Invoice No.', style: TextStyle(fontWeight: FontWeight.bold))),
  ...
  String _getSGST(item) => item.sgst.toStringAsFixed(2);
  String _getCGST(item) => item.cgst.toStringAsFixed(2);
  String _getIGST(item) => '-';

  List<PurchaseInvoice> _filterInvoices(List<PurchaseInvoice> invoices) {
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
